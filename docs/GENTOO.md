...
```text
https://github.com/escapechen/FriedasBirdview/archive/refs/tags/v1.9.0.tar.gz
```

Therefore a manually created `.tar.gz` or a GitHub Release is **not** required
for source installs. A pushed, immutable `v1.9.0` tag is required. The
...
```sh
git tag -s v1.9.0 -m "FriedasBirdview 1.9.0"
...
After `v1.9.0` exists on GitHub, generate the checksum manifest. This downloads
the source archive and records its hashes:

```sh
sudo ebuild /var/db/repos/friedasbirdview/net-misc/friedasbirdview/friedasbirdview-1.9.0.ebuild manifest
```
